...
### 1b. Governed Handoffs: Agent-to-Agent (A2A) and Tool/MCP Governance
...
### 1c. Where AI Judgment Stops and Deterministic Rules Take Over
...
### 1g. Planning vs. Execution Separation
...
No planning agent holds execution privilege at any point — this is the same segregation-of-duties principle in Sec. 1d, restated as an architectural boundary rather than a process rule.

### 1h. Enterprise Tool Registry

Every tool a component can call is registered, not just allow-listed at the agent level.

| Tool | Owner | Risk level | Approval status |
|---|---|---|---|
| CRM | Sales Ops | Low | Approved |
| Portfolio DB | Wealth Ops | Medium | Approved |
| Order Management System (OMS) | Trading Ops | Critical | Approved, restricted to Trading Agent |
| Compliance rules engine | Compliance | High | Approved |
| Risk analytics engine | Risk | Medium | Approved |
| Document repository | Legal | Medium | Approved |
| Research platform | Research | Low | Approved |
| Tax-lot engine | Tax Ops | Medium | Approved |
| Market data feeds | Market Data Ops | Low | Approved |
| Client portal | Client Experience | Medium | Approved |
...
Prompt templates, system prompts, and guardrails are maintained in a governed **Prompt Registry** with version control, approval history, ownership, and rollback capability — this is what makes the "system prompt update" row in the change classification table (Sec. 11a) enforceable rather than aspirational.

Each of the 15 components is also entered in an **Agent Registry**: agent ID, purpose, owner, approved tools, risk tier, model version, prompt version, and approval status — one record per component, kept current as any of those fields change.

### 1i. Confidence-Based Human Routing

A control on top of the value-threshold routing already in Sec. 1a: if FA-Assist's own confidence in a recommendation falls below a set threshold — independent of transaction value — it routes to mandatory human review before advisor approval, not just before execution. Low confidence and high value are two separate triggers for the same human gate.

### 1j. Memory Governance

Different memory types are governed differently — a single "agent memory" policy is not enough.

| Memory type | Governance |
|---|---|
| Conversation memory | Session-scoped, cleared per interaction unless retained under Sec. 1f |
| Working memory | In-flight only, never persisted across phases |
| RAG memory (vector store) | Governed under Sec. 1f, access scoped per client |
...
|---|---|---|
| AI-CNTRL-A01 | Unique scoped identity for every machine component; individually attributable identity for every human approver | F-01 remediation; A2A identity-per-hop (Sec. 1b) |
| AI-CNTRL-A02 | Credential scope = documented action scope | Confused-deputy check (Sec. 1b, 2) |
...
| AI-CNTRL-A08 | Oversight posture matches autonomy level | Autonomy Boundaries (Sec. 1a); all 5 sequence stages |
| AI-CNTRL-A10 | Incident containment incl. credential revocation | Remediation |
| AI-CNTRL-A11 | Vendor/model due diligence | Risk 4 |
| AI-CNTRL-A14 | Tool/MCP allow-listing per agent | Sec. 1b; Pre-Deployment Sections 03, 05 |
| AI-CNTRL-A16 | Vendor model version pinned | Risk 5 |

### 8a. Monitoring KPIs

Tracked by the Governance Monitoring Agent, thresholds set at deployment:
...
`ORG-AI-013` is a fifteen-component, nine-phase agentic wealth-management system classified Critical because one tightly controlled execution component is authorized to execute financial transactions.
...
| 6. Regression testing | Sec. 6a's test suite re-run against the fix |
...
### 11a. Continuous Post-Deployment Validation
...
Every field this document's evidence traces back to, in one record: AI system ID (`ORG-AI-013`), business owner, technical owner, vendor, model, risk classification, approval status, last review date, regulatory scope. This is the single row a firm-wide AI inventory would hold for this system — everything else in this document is the substantiation behind each field.
